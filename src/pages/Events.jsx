  ...
  return (
    <>
      <section className={`section | ${styles.event_section}`}>
        <h1 className='title'>
          <TextType text={[
            "Dive into the Tech Universe with us",
            "From AI talks to coding bootcamps, we bring real-world learning to campus.",
            "Join us and transform your curiosity into hands-on experience."
          ]} />
        </h1>
        <div className={`flex_center | ${styles.event_container}`}>
        {eventsData.slice(0, count ? count : eventsData.length).map((event, index) => (
            <motion.div
              className={`flex_center | ${styles.event_content}`}
              key={index}
              custom={index}
              initial="hidden"
              whileInView="visible"
            ...
            >
              ...
              <div className={styles.event_right_content}>
                <h2>
                  <ShinyText text={event.title} disabled={false} speed={3} className='custom-class' />
                </h2>
                <ScrambledText className="text-xl">{event.description}</ScrambledText>
